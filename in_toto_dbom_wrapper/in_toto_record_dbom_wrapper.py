...
"""
<Program Name>
  in_toto_record_dbom_wrapper.py

<Author>
  Ryan Mathison <Ryan.Mathison2@unisys.com>

<Purpose>
  Provides a wrapper for command line interface for runlib.in_toto_record
...
LOG = logging.getLogger("in_toto_record-wrapper")
...
def main():
  # Define the args needed for dbom
  child_parser = argparse.ArgumentParser(parents=[], add_help=True,
      epilog=textwrap.dedent('''\
         IMPORTANT:
            This command wraps the in-toto-record command
  ...
  #define the in-toto arg values we need to know about
  ...
  #pass the non-dbom args onto in-toto-record
  LOG.debug("childArgs")
  LOG.debug(child_args)
  LOG.debug(rest)
  rest_arg = ['in-toto-record']
  rest_arg.extend(rest)
  sys.argv = rest_arg
  #subprocess.Popen(rest_arg).wait()

  #run the in-toto-record command and see if it worked
  with subprocess.Popen(rest_arg, stdout=subprocess.PIPE) as proc:
    ...
    #now read the step name so we can find the in-toto link
    args, rest = parent_parser.parse_known_args()

    #find the latest in-toto link for the step
    try:
      ...
      #open the link and write it to the dbom record
      ...
      LOG.info("---------- In-Toto-Record Start - No DBoM Update ----------")
